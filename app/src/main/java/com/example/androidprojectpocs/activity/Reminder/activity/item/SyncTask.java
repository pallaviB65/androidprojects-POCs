package com.example.androidprojectpocs.activity.Reminder.activity.item;
...
public class SyncTask extends AsyncTask<Void, Void, Void> {
    ...
    private DatabaseProcess databaseProcess = new DatabaseProcess(ReminderActivity.context);
    ...
    private void makeSync() throws IOException {
        ...
        do {
            ...
            for (CalendarListEntry calendarListEntry : items) {
                if (calendarListEntry.getId()
                        .equals(ReminderActivity.sharedPreferences.getString(ReminderActivity.CAL_ID, ""))) {
        ...
        if (!isExisted) {
            ...
            calID = created.getId();
            ReminderActivity.sharedPreferences.edit().putString(ReminderActivity.CAL_ID, calID).apply();
        }
        //160715
        calId = calID;
        pageToken = null;
    ...
    protected void onPostExecute(Void output) {
        switch (function) {
            case Constants.TASK_SYNC:
                ...
                ReminderActivity.recyclerAdapter2.updateData(ReminderActivity.rearrangeList(events));
                break;
            case Constants.TASK_DELETE:
                break;
            case Constants.TASK_MODIFY:
        ...
        if (mLastError != null) {
            if (mLastError instanceof GooglePlayServicesAvailabilityIOException) {
                ...
                Dialog dialog = apiAvailability.getErrorDialog(
                        ...
                        ReminderActivity.REQUEST_GOOGLE_PLAY_SERVICES);
                dialog.show();
            } else if (mLastError instanceof UserRecoverableAuthIOException) {
                activity.startActivityForResult(
                        ((UserRecoverableAuthIOException) mLastError).getIntent(),
                        ReminderActivity.REQUEST_AUTHORIZATION);
            } else {
                Toast.makeText(ReminderActivity.context, "The following error occurred: "
                        + mLastError.getMessage(), Toast.LENGTH_LONG).show();
                System.out.println(mLastError.getMessage());
            }
        } else {
            Toast.makeText(ReminderActivity.context, "Request cancelled.", Toast.LENGTH_LONG).show();
        }
    }

    protected void onPreExecute() {
        calId = ReminderActivity.sharedPreferences.getString(ReminderActivity.CAL_ID, "");
        mService = new com.google.api.services.calendar.Calendar.Builder(
                transport, jsonFactory, ReminderActivity.mCredential)
